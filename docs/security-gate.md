...
출시 전 P0 항목은 모두 통과해야 한다. 2026-06-25 현재 로컬 기준선은 `node --check workers/api/src/index.ts`, `pnpm test -- tests/cloudflare-api.test.ts` 115 passed, `pnpm audit --audit-level critical`, `git diff --check`가 통과한 상태다. Staging D1 `0002` migration/seed 증적은 원격 적용까지 완료됐고, Cloudflare staging/production URL, R2 활성화, production D1 `0002` migration 증적은 외부 blocker로 별도 남아 있다.

## P0 출시 차단 항목

- [x] 현재 기준선에서 `pnpm test -- tests/cloudflare-api.test.ts`가 115 passed 상태다.
