...
출시 전 P0 항목은 모두 통과해야 한다. 2026-07-20 로컬 코드는 전체 테스트, typecheck, lint, production build와 보안 하네스를 통과했다. 수정된 읽기 전용 계정 검사는 이전의 다중 SQL 오류 순서 결함을 제거했다. Backup-gated recovery는 비어 있던 구형 source table을 별도 이름으로 보존한 뒤 정상 Wrangler migration `0018`~`0026`을 적용했고, 최신 후속 검증은 pending 0건·migration registry 일치·core seed 증거 통과·검증 쓰기 0건을 확인했다. 전용 staging/production `COST_GUARD_STATE` KV는 별도 생성·바인딩됐고 exact-host Turnstile widget·public key·staging secret version까지 준비됐지만, R2 사용자 checkout, post-`0022` live R2/D1 reconciliation, WAF/static-routing evidence, Turnstile live activation/smoke와 production secret, API Worker/URL과 exact API origin, NAVER용 소유 custom domain, production D1, source 권리, 운영 채널, native 실기기, 법무 서명이 없어 출시 상태는 계속 `blocked-external`이다.
...
| --- | --- | --- | --- |
| Staging D1 passes through `0026`; production lacks externally verified V2 migrations | data-operations | before production candidate | latest staging boundary, no-pending Wrangler list, migration registry, and core seed evidence pass. A protected pre-change external backup remains preserved. Post-`0022` uploads remain stopped until live R2/D1 reconciliation. Production remains `D1_0006_NOT_APPLIED` |
| Turnstile staging config is provisioned, but live activation/smoke and the production secret are pending | release-operator + security owner | before staging release candidate | the exact-host widget, public key, and staging secret version exist; photo upload remains fail-closed until the API Worker activates the version and server-verifiable success/failure evidence passes |
...
| Staging API secret target exists, but configured application deployments and healthy staging/production URLs are missing | release-operator | before staging release candidate | `/api/health` currently returns 404; no live smoke or production readiness claim |
...
| Staging `0023`/`0024` schema evidence passes; the proof-required API and dedicated 3/minute first-line limiter are not live | identity/security owner | before public launch | require `SILSIGAN_ANON_SESSION_REQUIRED=1`, `SILSIGAN_ANON_SESSION_DAILY_LIMIT=5000`, `ANONYMOUS_SESSION_RATE_LIMITER`, owner-domain live issue/limit/rotation/revocation/wrong-proof/stolen-ID evidence, and select member/device binding if complete-credential theft must also be resisted |
