...
출시 전 P0 항목은 모두 통과해야 한다. 최신 active truth는 root `454/454`, mobile `4/4`, lint·typecheck·Next/OpenNext build 통과다. Staging API/web은 Workers KV 과소비 경로를 수정해 재배포했고 D1은 `0026`까지 정렬됐으며 private R2는 마지막 대조에서 `0 B`다. 사람 Turnstile 사진 upload/moderate/read/delete와 zero-residual 증거, WAF/static-routing, 운영 채널, native 실기기, 법무 서명, production은 여전히 열려 있다. Production 변경은 없다. 수동 source/diff 보안 검토, 비밀 패턴 검사, 의존성 감사, negative-path 회귀는 통과했다. Codex Security scan `7fe0ad74-161b-44e7-bbfb-fb0eeb07c6c2`도 15/15 범위를 완료했고, 발견한 한 건의 Medium 클라이언트 캐시 문제는 source `656eb315cbde4505b6c7db342a0185bb2762baea`에서 수정·재검증됐다. 아래의 이전 `422`, `R2_NOT_ENABLED`, API 미배포 기록은 명시된 과거 시점의 증거일 뿐 최신 상태가 아니다.
...
- Client Safety State: PASS LOCAL - only map/search/bounds/background refreshes can select the low-cost `places_status` scope. Creator block/unblock, both moderation-report variants, and account deletion fully reconcile visibility-dependent caches before success; deletion also clears owned UGC caches synchronously. The original source-state PoC no longer matches the fixed source, and an independent security re-review found no remaining actionable issue in the scoped patch.
- Tests: PASS LOCAL - source `656eb315cbde4505b6c7db342a0185bb2762baea` root set `454/454` and mobile set `4/4` passed, 0 failed, 0 skipped. The previously committed `422/422` result is retained only as historical evidence; remote branch-tip equality is verified separately after push.
...
| --- | --- | --- | --- |
| Client reconciliation regression is covered by source-contract tests rather than an isolated mounted React harness | frontend + security owner | before wider staging UGC beta | keep the explicit refresh-scope and success-order contracts; add a mounted block/moderation regression when a repository-native component harness exists, and do not substitute provider mutation for the pending human-approved staging exercise |
| Rolling Workers KV usage still includes the pre-remediation window | engineering + Cloudflare operator | after next `09:00 KST` reset and before wider staging use | confirm the new slope under one visible leader tab; if reads do not materially fall, close staging traffic and investigate cron, external bots, or other namespace consumers |
| Retired Turnstile secret can remain valid during Cloudflare's rotation grace period | security owner | two hours after 2026-07-22 rotation | keep upload exposure limited to staging, monitor failures, and do not reuse or record either value; public config must continue to expose only the site key |
| Staging D1 passes through `0026`; production lacks externally verified V2 migrations | data-operations | before production candidate | latest staging boundary, no-pending Wrangler list, migration registry, and core seed evidence pass. A protected pre-change external backup remains preserved. Production remains `D1_0006_NOT_APPLIED` |
| Managed Turnstile is live but the human photo lifecycle and production secret are pending | release-operator + security owner | before staging release candidate | a bounded JPEG reached the exact-host challenge, but automation was rejected before token issuance. Require human upload/moderate/read/delete and R2/D1 zero-residual proof without weakening the widget |
| Private staging R2 is active; real photo privacy/deletion evidence is pending | release-operator + data-operations | before staging release candidate | bucket privacy/lifecycle and `0 B` reconciliation pass. Require one human Turnstile lifecycle and return to zero residual; production R2 remains unchanged |
| Staging API/web are live; production API and owner-domain URLs remain missing | release-operator | before production candidate | staging health and live data mode pass. Do not infer production readiness or traffic promotion |
...
| Staging proof-required identity controls are live; owner-domain replay/rotation evidence remains open | identity/security owner | before public launch | retain `SILSIGAN_ANON_SESSION_REQUIRED=1`, `SILSIGAN_ANON_SESSION_DAILY_LIMIT=5000`, `ANONYMOUS_SESSION_RATE_LIMITER`; add owner-domain live issue/limit/rotation/revocation/wrong-proof/stolen-ID evidence and select member/device binding if complete-credential theft must also be resisted |
...
## 2026-07-20 잔여 위험 스냅샷 — historical/superseded

이 표는 당시 차단 상태와 명령 계약을 보존한다. 현재 active truth는 문서 상단과 `release-ledger.yaml`을 따른다. 특히 staging API/web, private R2, D1 `0026`은 활성 상태이며, 남은 사진 게이트는 사람 Turnstile 전체 생명주기와 zero-residual 증거다.
...
- R2/deployment external-state 결과. `pnpm cf:external-state`는 Wrangler OAuth, R2 bucket visibility, 각 configured bucket의 disabled `r2.dev` URL과 absent direct R2 custom domain, staging/production deployment URL shape, staging/production Worker dry-run, staging/production D1 evidence를 non-mutating JSON check로 분리한다. **2026-07-20 historical baseline**은 OAuth pass, staging/production dry-run pass, deployment URL 4개와 exact API origin missing fail, Staging D1 through `0026` pass, Production `D1_0006_NOT_APPLIED`, `cloudflare.r2.enabled` fail with `R2_NOT_ENABLED`였다. 이 R2/API staging blocker는 이후 해소됐으며 현재 상태는 문서 상단을 따른다. `pnpm cf:r2:evidence` 기본 실행은 non-mutating plan-only이고, `--check`는 bucket list, `r2 bucket dev-url get`, `r2 bucket domain list`만 읽는다. `--apply`는 누락 bucket 생성 후 동일한 visibility/privacy evidence를 재확인하며 Production apply는 `--confirm-production` 없이는 실패한다.
- D1 migration registry 안전성 결과. 2026-07-20 수정된 read-only `pnpm cf:d1:evidence -- --env=staging --check`는 직접 테이블을 읽기 전에 `sqlite_schema`와 `pragma_table_info`만으로 경계를 검사한다. Backup-gated recovery는 0행 구형 table을 archive한 뒤 정상 Wrangler migration `0018`~`0026`을 적용했다. 최신 후속 실행은 Staging 스키마 through `0026`, Wrangler pending 0건, migration registry 일치, core seed 증거를 모두 통과했고 별도 audit query도 legacy archive 1개·0행·검증 writes 0을 확인했다. `--apply`는 migration list 뒤 이 경계를 먼저 대조하며 불일치하면 `migrations apply` 전에 중단한다. 보호된 pre-change 외부 backup은 보존되어 있다.
...
- D1 operator 순서 보강. 정상 apply 계획은 migration list → schema-only boundary/registry 일치 확인 → migrations apply → idempotent seed → post-apply migration list → 최종 schema/seed 및 registry 확인 순서다. Release gate blocker 수집기는 중첩 `results[].check`를 재귀적으로 읽는다. 현재 Staging은 migration blocker 없이 통과하고 Production의 `D1_0006_NOT_APPLIED`만 요약된다.
