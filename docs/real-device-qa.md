...
- Post-deploy rerun preflight: USB/Xcode recognizes one physical iOS destination, Xcode Apple Accounts login is complete, and credential fields are closed. The signed-in account has two team candidates, while the single local development identity matches neither; retry with provisioning updates therefore stopped before build with missing profiles. Failed temporary result bundles and raw logs were deleted immediately because they could contain device/signing identifiers. No credential or 2FA value is retained.
