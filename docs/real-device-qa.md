...
Updated: 2026-07-27
Status: staging API/web and private R2 are live. An Apple Development-signed staging build compiles, installs over the existing app data, and launches on a cable-connected physical iPhone 12 Pro running iOS 26.5.2. The exact HTTPS staging URL loads the live home without a startup crash, and the first-screen screenshot is retained below. The `AppUITests` target builds, signs, installs, and executes on the physical phone: 3 tests passed, 1 fail-closed photo-rights test was skipped because the device was outside the 300 m radius of a registered staging place, and 0 tests failed. No automated test selected or transmitted a photo. The run exposed an upload-map fallback layout defect; the shared bounded mobile-map styles and a regression test are now present locally, while Cloudflare staging redeployment and post-deploy iPhone visual proof remain pending explicit external-transmission approval. TestFlight release signing, the remaining native mutation/deletion matrix, and Android real-device QA also remain pending.
...
| TestFlight build | not selected; App Store Connect login and current build-state inspection require the account holder |
...
- Local remediation: both upload-location map wrappers reuse the existing bounded `.realMapFrame` styles with a 190 px mobile height. The UI contract regression, lint, typecheck, production build, and full root suite (`464/464`) pass. Cloudflare staging redeploy and a fresh iPhone screenshot remain required before closing the visual issue.
