...
const travelGuidePath = resolve(testDir, "../src/lib/travel-guide.ts");
...
const travelGuideSource = readFileSync(travelGuidePath, "utf8");
...
test("static directory mode preserves place discovery while stopping live reads, writes, and telemetry", () => {
  ...
  assert.match(redesignSource, /dataMode === "directory" \? "실시간 연결 다시 시도" : "이 지역 다시 검색"/);
...
test("place detail fetches current evidence and renders source plus provider observation time", () => {
  ...
  assert.match(placeScreen, /buildGroundedTravelGuide/);
  assert.match(placeScreen, /AI 여행 변수 대응 · 안전모드/);
  assert.match(placeScreen, /guide\.title/);
  assert.match(placeScreen, /guide\.ctaLabel/);
  assert.match(placeScreen, /guide\.decision === "insufficient" && nearbyPlaces\.length === 0/);
  assert.match(placeScreen, /onGoMap\(\)/);
  assert.match(placeScreen, /officialTourismPlace/);
  assert.match(placeScreen, /한국관광공사 TourAPI/);
  assert.match(travelGuideSource, /"지금 가기"/);
  assert.match(travelGuideSource, /"조금 기다리기"/);
  assert.match(travelGuideSource, /"근처 대안 보기"/);
